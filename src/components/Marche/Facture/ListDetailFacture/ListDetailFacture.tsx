...
type ListDetailFactureProps = {
  //
};


const ListDetailFacture: React.FC<any> = () => {
  ...
  const getCols = async() => {
    await axios.get(`${process.env.REACT_APP_API_BASE_URL}/forms/dfacture/`,{
      headers: {
  ...
  const getRows = async(url:string) => {
    ...
    await axios.get(`${process.env.REACT_APP_API_BASE_URL}/sm/detail/?facture=${fid.facture}&${url}`,{
      headers: {
        Authorization: `Token ${Cookies.get('token')}`,
        'Content-Type': 'application/json',
                    ...
                    <div className="card-body" >
                      ...
                      <div className="card" style={{ height:'90px',width: "40%",background:'#ebebeb' }}>
                        <div className="card-body text-center">
                          <h5 className="text-center card-title">Details de la facture</h5>
                      ...
                      >
                        <>
                          <DisplayDataGridModal img={bill} title={"Detail Facture"} cols={cols}   />
...
export default ListDetailFacture;
