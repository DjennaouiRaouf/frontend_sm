...
const AddAvance: React.FC<any> = () => {
    const [validated, setValidated] = useState(false);
    const { variant,show,heading,text } = useSelector((state: RootState) => state.alertReducer);
    const { showAddForm2,pk2 } = useSelector((state: RootState) => state.addDataGridModal);
    ...
    const getDdfaultState = async() => {
        await axios.get(`${process.env.REACT_APP_API_BASE_URL}/forms/avancefieldsstate/`,{
            headers: {
                Authorization: `Token ${Cookies.get("token")}`,
                'Content-Type': 'application/json',
                                        ...
                                        <div className="row">
                                            <div className="row">
                                                <div className="col-md-12 text-start">
                                                    <div className="mb-5">
                                                        <h1 className="text-center">{`Avance du Marche ${pk2}` }</h1>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
